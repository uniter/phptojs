...
describe('Transpiler loose inequality comparison operator test', function () {
    ...
    it('should correctly transpile a return with a comparison using "<>" between a variable and complex expression', function () {
        var ast = {
            name: 'N_PROGRAM',
            statements: [{
                name: 'N_RETURN_STATEMENT',
        ...
        expect(phpToJS.transpile(ast, {bare: true})).to.equal(
            'function (core) {' +
            'var createString = core.createString, getVariable = core.getVariable, isNotEqual = core.isNotEqual, snapshot = core.snapshot, ternary = core.ternary;' +
            'return isNotEqual(' +
